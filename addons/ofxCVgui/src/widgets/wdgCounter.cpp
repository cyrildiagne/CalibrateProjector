...
void wdgCounter::setMax(unsigned int myMax)
{
	...
	_height = 20+COUNTER_RADIUS*4*rows;
	_max = myMax;
}
...
void wdgCounter::draw()
{
	...
	ofDrawBitmapString(caption + ":", _x, _y+5);
	ofDrawBitmapString(numDisplay, _x+_width - numDisplay.length()*8, _y+5);
	...
	for (unsigned int i=0; i < maxCounters; i++)
	{
		...
		ofCircle(_x + col * COUNTER_RADIUS*3,
				 _y+10+COUNTER_RADIUS + row * COUNTER_RADIUS*3, COUNTER_RADIUS);
	}
