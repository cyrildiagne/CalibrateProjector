...
scrPreviewRGB("RGB camera", kinect.getimageTextureReference()),
...
wdgClear("Clear")
{
    ...
    scrCaptureControls.push(new wdgButton("Use Calibration", scrKinectView.bUseCalibration));
...
void testApp::update(){
	...
	board.findCorners(kinect.getImagePixels(), foundCornersC);
    ...
    if(calibrate.ready) {
        scrKinectView.intrinsics = &calibrate.intrinsics;
        scrKinectView.modelMatrix = &calibrate.modelMatrix;
    }
...
void testApp::drawFoundCorners3D(ofNode &n){
    ...
    if(scrKinectView.bUseCalibration) return;
